...
export default function AgentComplaintsPage() {
  ...
  const fetchComplaints = async () => {
    setLoading(true);
    try {
      const response = await apiClient.getAgentComplaints(pagination.limit, pagination.offset);
      let filteredComplaints = response.complaints || [];

      if (statusFilter !== 'all') {
        filteredComplaints = filteredComplaints.filter((c: ComplaintData) => c.status === statusFilter);
      }
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {complaints.map((complaint) => (
                    ...
                    >
                      <td className="px-6 py-4 text-2xl">{getTypeIcon(complaint.type)}</td>
